...
| backend.environment.publisherDomain | string | `""` | Hostname only for issuer `did:web` IDs from `issuers.yaml` aliases (not a URL). When empty, taken from `DID_WEB_SERVER_URL`. |
